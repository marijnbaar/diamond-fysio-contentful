...
function getVercelProjectConfig() {
  // NOTE: This file is imported by Next.js pages/routes.
  // Do not import Node.js built-ins (fs/path) here, as Turbopack may try to bundle them.
...
export async function loadPosts() {
  if (typeof window !== 'undefined') {
    console.error('loadPosts should only be called on the server side');
    return { data: [] };
  }

  let secret = process.env.INSTAGRAM_API_KEY;
  if (!secret) {
    console.error('INSTAGRAM_API_KEY environment variable is missing');
    return { data: [] };
  }
    ...
    if (!response.ok) {
      ...
      if (response.status === 400 || response.status === 401) {
        ...
        if (isInvalidToken) {
          ...
          console.error('5. Update INSTAGRAM_API_KEY in Vercel with the new token');
        } else {
...
export async function manualTokenRefresh() {
  if (typeof window !== 'undefined') {
    throw new Error('manualTokenRefresh must be called on the server side');
  }
