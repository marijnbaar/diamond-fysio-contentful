...
const getTypeName = async (slug, preview, query) => {
  try {
    ...
    if (match.__typename === 'Aboutpage' && match.pageType) {
      // Als pageType is 'teampage', return 'Teammemberpage' (modelId blijft hetzelfde)
      if (match.pageType === 'teampage') {
  ...
  try {
    ...
    if (modelId === 'Homepage') {
      ...
      if (locale) {
        ...
        // Filter op pageType om alleen teampage entries te krijgen
        const baseMatch = baseItems.find((it) => it.slug === slug && it.pageType === 'teampage');
        const targetId = baseMatch?.sys?.id;
        if (targetId) {
          target =
            page.find((it) => it?.sys?.id === targetId && it.pageType === 'teampage') || null;
        }
      }
      // Fallback: localized slug match when id was not resolved
      if (!target) {
        target = page.find((item) => item.slug === slug && item.pageType === 'teampage') || null;
      }

      if (!target || !target.teamMember || target.pageType !== 'teampage') {
        return {
          notFound: true
        };
      }
