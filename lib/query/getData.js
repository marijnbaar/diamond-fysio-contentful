...
const getPage = async (modelId, slug, preview, locale = 'nl') => {
  try {
    ...
    if (modelId === 'Homepage') {
      let pageData = data[`${modelId.toLowerCase()}Collection`].items;
      ...
      const pageData = data[`aboutpageCollection`].items;
  ...
  try {
    ...
    if (!errorResult) {
      if (retryCount < MAX_RETRIES) {
        ...
        console.warn(
          `[Deprecation Warning] getComponent reached max retries (${MAX_RETRIES}) for modelId: ${modelId}, id: ${id}. Returning null.`
        );
        return null;
      }
    }
